...
- [ ] UptimeRobot or similar external monitoring before going public
...
2. **Debug `/analytics/dashboard` 500** — only verified backend bug. Generic error handler hides the trace; needs to be triggered from inside the Fly container with `flyctl logs` tailing.
